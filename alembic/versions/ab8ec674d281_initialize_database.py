...
def upgrade():
    ...
    sa.Column('created', sa.TIMESTAMP(), server_default=sa.func.now(), nullable=False),
    ...
    sa.Column('type', sa.Enum(u'c', u'd', u'f', u'l', u's'), nullable=False),
    sa.Column('links', sa.INTEGER(), server_default=sa.text(u'1'), nullable=False),
    sa.Column('sparseness', sa.Float(), server_default=sa.text(u'1'), nullable=False),
    sa.Column('shasum', sa.VARBINARY(length=64), nullable=True),
    sa.Column('first_backup', sa.TIMESTAMP(), server_default=sa.func.now(), nullable=False),
    ...
    op.create_index('index3', 'files', ['filename', 'path', 'host_id', 'mode', 'size', 'mtime', 'uid', 'gid'], unique=True)
    op.create_index(op.f('ix_files_host_id'), 'files', ['host_id'], unique=False)
    op.create_table('savesets',
    sa.Column('id', sa.INTEGER(), autoincrement=True, nullable=False),
    ...
    op.create_index(op.f('ix_savesets_backup_host_id'), 'savesets', ['backup_host_id'], unique=False)
    op.create_index(op.f('ix_savesets_finished'), 'savesets', ['finished'], unique=False)
    op.create_index(op.f('ix_savesets_host_id'), 'savesets', ['host_id'], unique=False)
    ...
    sa.Column('created', sa.TIMESTAMP(), server_default=sa.func.now(), nullable=False),
    sa.Column('removable', sa.BOOLEAN(), server_default=sa.text(u'0'), nullable=False),
    sa.Column('mounted', sa.BOOLEAN(), server_default=sa.text(u'1'), nullable=False),
    sa.Column('host_id', sa.INTEGER(), server_default=sa.text(u'0'), nullable=False),
    sa.ForeignKeyConstraint(['host_id'], [u'hosts.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('id'),
    sa.UniqueConstraint('volume')
    )
    op.create_index(op.f('ix_volumes_host_id'), 'volumes', ['host_id'], unique=False)
    ...
    sa.ForeignKeyConstraint(['saveset_id'], [u'savesets.id'], ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['volume_id'], [u'volumes.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('id')
    )
    op.create_index(op.f('ix_backups_file_id'), 'backups', ['file_id'], unique=False)
    op.create_index(op.f('ix_backups_saveset_id'), 'backups', ['saveset_id'], unique=False)
    op.create_index(op.f('ix_backups_volume_id'), 'backups', ['volume_id'], unique=False)
